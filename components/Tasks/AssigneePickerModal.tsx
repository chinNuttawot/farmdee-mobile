...
export default function AssigneePickerModal({
  ...
  onConfirm: (cfgs: AssigneeConfig[]) => void;
  onResetInitial: () => void; // เรียกเมื่อปิดเพื่อเคลียร์ค่า default รอบถัดไป
}) {
  const [cfgs, setCfgs] = useState<AssigneeConfig[]>(initial);
  useEffect(() => setCfgs(initial), [open, initial]);
                  ...
                  >
                    {c.isDaily ? (
                      <TextInput
                        ...
                        value={c.pricePerDay ?? ""}
                        onChangeText={(v) =>
                          setField(i, "pricePerDay", v.replace(/[^0-9.]/g, ""))
                        }
